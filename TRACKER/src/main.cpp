...
void loop() {
    ...
    if (deviceConnected) {
        ...
        // bool danneggiato = outputs[0] > 0.8 ? true : false;
        bool danneggiato = random(0,2);
        pHighValuesCharacteristic->setValue(std::to_string(danneggiato));
        pHighValuesCharacteristic->notify();
        Serial.printf("Danneggiamento: %d \n", danneggiato);
